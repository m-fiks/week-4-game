...
const clickToStart = function(){
    ...
    //jewel buttons get new values 03/08 --- try to get rid of 0 being assigned to buttons~!!!!!!!!!!!!!
    crystalButton=[Math.floor(Math.random()*buttonValue.length)] +1;
    purpleButton=[Math.floor(Math.random()*buttonValue.length)] +1;
    blueButton=[Math.floor(Math.random()*buttonValue.length)] +1;
    greenButton=[Math.floor(Math.random()*buttonValue.length)] +1;
    console.log(parseInt(blueButton),parseInt(crystalButton), parseInt(purpleButton),parseInt(greenButton));
};
